...
1. **🌐 全球学术检索（搜什么出什么）**：
   ...
   * 搜索栏可选**泛搜**（输入较宽的学科主题）或**精搜**（输入更具体的研究问题或多个条件）。两种方式都会核对标题或摘要的主题相关性；精搜要求详细查询的关键要点均被覆盖，不要求标题逐字一致。输入主题后，切换**「最新文献 / 近 30 天热门 / 经典文献」**查看不同结果。按最新排序时，每批检查至多 200 条候选并合并同名版本；索引候选总量并非筛选后的结果总量。OpenAlex 不可用时，Crossref 备用结果也会逐条核对，不展示未核对的原始命中总数。
   * 「近 30 天热门」从主题候选中查询**过去 30 天发表的论文对它们的引用次数**并据此排序，候选论文自身可以发表于更早年份。此榜单只覆盖有限候选集，不是全库热度排名；OpenAlex 收录与引用更新可能滞后。引用数据不可用时不会以累计被引冒充近期热度。
   * OpenAlex 匿名接口在高负载时可能临时限流。可在本机环境变量 `SSB_OPENALEX_API_KEY` 中设置[免费的 OpenAlex API 密钥](https://openalex.org/settings/api)；程序通过请求头发送，不写入仓库。没有密钥也可以使用，但热门榜在限流时会明确显示暂不可核对。
   * 「经典文献」要求标题匹配主题或相关领域方法、发表满 5 年，再按被引次数排序，展示的是可供进一步核对的高被引候选，并非人工评定的必读清单。少数常见主题增加了方法词识别，其余主题仍按查询词匹配。
...
| 区域 | 说明 |
|---|---|
| **顶栏** | 左边可设置或切换定向追踪方向，右边是账号入口；选定方向后可手动更新文献 |
| **检索框** | 支持 **「本方向雷达」** 与 **「全球学术检索」**：在雷达模式下检索本地方向文献（多词为与，支持词组与前缀）；在全球模式下可按任意学科主题选择泛搜或精搜，并切换最新文献、近 30 天热门与经典文献 |
